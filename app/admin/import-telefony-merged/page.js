    ...
    <main className="max-w-[1000px] mx-auto px-6 py-10">
      <div className="flex items-center justify-between mb-6">
        <p className="text-lg font-medium">
          <span className="text-brand-ink">Import: nowa baza telefonów</span>{" "}
      ...
      <p className="text-xs text-brand-muted mb-6">
        Robi z <code className="text-[11px] bg-brand-cream px-1 py-0.5 rounded">data/telefony_merged.json</code>{" "}
        jedyne źródło prawdy dla kategorii telefony: dopasowane po nazwie produkty są aktualizowane w miejscu
      ...
      {preview.unmatchedCount > 0 && (
        <details className="mb-4 border border-brand-border rounded-lg bg-white p-3">
          <summary className="text-xs font-medium cursor-pointer">
            {preview.unmatchedCount} nowych telefonów (bez werdyktu/opisu — do uzupełnienia później)
          </summary>
