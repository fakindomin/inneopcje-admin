...
// inconsistency this was rewritten to remove. Ekran is a real priority
// here on equal footing with aparat/wydajność/bateria; which DIRECTION
// (kompaktowy vs duży) is asked separately in the dedicated
// "rozmiar_ekranu" step below, since picking ekran as a priority only
// says it matters, not which way.
const PRIORYTET_OPTIONS = [
  { id: "ekran", label: "Ekran", tag: "rozmiar_ekranu" },
  { id: "aparat", label: "Aparat", tag: "aparat" },
  { id: "wydajnosc", label: "Wydajność", tag: "wydajnosc" },
...
const ROZMIAR_OPTIONS = [
  ...
  { id: "brak", label: "Rozmiar nie ma znaczenia", tag: null },
];
...
export function resolvePath(answers, producentByTier = {}) {
  ...
  function addTagPick(tag) {
    if (["aparat", "bateria", "wydajnosc", "rozmiar_ekranu"].includes(tag) && !tagPicks.includes(tag)) {
      tagPicks.push(tag);
    }
  }
  ...
  const priorytet1 = push("priorytet1", "Co jest dla Ciebie najważniejsze?", PRIORYTET_OPTIONS);
  ...
  if (chosen1.tag) {
    ...
    // rozmiar_ekranu's own "resolved" state means "direction captured",
    // which only the dedicated step below can do - being picked as a
    // priority here says it matters, not which way, so it must NOT skip
    // that step.
    if (chosen1.tag !== "rozmiar_ekranu") resolvedTags.add(chosen1.tag);
  }

  if (!chosen1.avoidance) {
    const remaining = PRIORYTET_OPTIONS.filter((o) => o.id !== priorytet1 && !o.avoidance);
    if (remaining.length > 0) {
      const priorytet2Options = [...remaining, { id: "brak2", label: "Nic więcej się nie liczy", tag: null }];
      ...
      if (chosen2.tag) {
        ...
        if (chosen2.tag !== "rozmiar_ekranu") resolvedTags.add(chosen2.tag);
      }
    }
  }
