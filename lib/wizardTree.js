...
const PRIORYTET_OPTIONS = [
  { id: "ekran", label: "Ekran", tag: "jakosc_ekranu" },
...
// duzy/brak) by the dedicated step below, not through `.tag` like the
// priority options - these entries carry no tag on purpose, since size is
// never a tagPicks priority, only the late screenValue() tiebreak in
// lib/wizardMatch.js.
const ROZMIAR_OPTIONS = [
  { id: "kompakt", label: "Kompaktowy — łatwo mieści się w dłoni" },
  { id: "duzy", label: "Duży — do multimediów i gier" },
  { id: "brak", label: "Rozmiar nie ma znaczenia" },
];
