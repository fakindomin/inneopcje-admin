...
// How many producers the "producent" step lets someone pick at once.
...
const PRIORYTET_OPTIONS = [
  { id: "ekran", label: "Świetny ekran – żywe kolory i jasność w słońcu.", tag: "jakosc_ekranu" },
  { id: "aparat", label: "Dobry aparat – ostre zdjęcia i wideo", tag: "aparat" },
  { id: "bateria", label: "Mocna bateria – intensywny dzień bez ładowarki.", tag: "bateria" },
  { id: "wydajnosc", label: "Błyskawiczne działanie – brak przycięć w czasie pracy", tag: "wydajnosc" },
];
...
const PRIORYTET2_LABELS = {
  aparat: "Ładny, ostry kadr gotowy od razu po wyjęciu z kieszeni.",
  bateria: "Spokój o to, że bateria wytrzyma do końca dnia.",
  wydajnosc: "Płynne działanie przy wielu zadaniach naraz.",
};
...
const HUMAN_OPTIONS = [
  { id: "zdjecia", label: "Robię zdjęcia i przeglądam social media.", tag: "aparat" },
  { id: "ruchu", label: "Dużo dzwonię, piszę i rzadko mam pod ręką ładowarkę.", tag: "bateria" },
  { id: "rozne", label: "Klasyczny mix – internet, multimedia i codzienne wiadomości.", tag: null },
];
...
const ROZMIAR_OPTIONS = [
  { id: "kompakt", label: "Kompaktowy – obsługa jedną dłonią i mieści się w kieszeni." },
  { id: "sredni_rozmiar", label: "Złoty środek – uniwersalny rozmiar do codziennego użytku." },
  { id: "duzy", label: "Duży ekran – maksymalna przestrzeń do filmów, gier i multimediów." },
];
...
//
// multiSelect steps (currently just "producent") store an ARRAY of ids
// instead of one - the same self-validating rule applies per-entry (drop
// any id that isn't a current option), and the step only counts as
// answered once at least one entry survives. maxSelect is carried onto the
// returned step so the UI (components/WizardStep.js) knows the cap.
...
// screenRangeByTier: { budzetowy: bool, sredni: bool, premium: bool } -
// whether that tier's recent catalog actually spans a real screen-size
// range (see lib/wizardScreenRange.js). Defaults to empty, which is treated
// as "yes, ask" (see below) so this stays callable without it too.
export function resolvePath(answers, producentByTier = {}, screenRangeByTier = {}) {
  const steps = [];
  ...
  // TEST/temporary (see app/api/wizard-live-count): stamps whatever budzet/
  // tags/marka/producent are known so far onto the current last step, no
  // matter how far the walk got - so the "how many models match" counter
  // can read a real, single-source-of-truth snapshot instead of
  // re-deriving this same walk itself. Safe to strip alongside the counter
  // route/component once the test is done: nothing else reads
  // profileSoFar.
  function finish() {
    const last = steps[steps.length - 1];
    if (last) last.profileSoFar = { budzet, tags: [...tagPicks], marka, producent };
    return steps;
  }

  const budzet = push("budzet", "Jak zazwyczaj podejmujesz decyzje przy zakupie sprzętu na lata?", BUDZET_OPTIONS);
  if (!budzet) return finish();

  const priorytet1 = push(
    "priorytet1",
    "Gdy myślisz o nowym telefonie, co ma dla Ciebie bezwzględny priorytet?",
    PRIORYTET_OPTIONS
  );
  if (!priorytet1) return finish();
  ...
  if (marka === "tak") {
    ...
    // premium). See lib/wizardBrands.js for how this is fetched. No escape
    // hatch to "something else" on purpose: this question only fires after
    // the user already said they specifically want a known, trusted brand
    // (see MARKA_OPTIONS "tak"), so offering an "unusual" option here would
    // contradict what they just answered — that path is marka="niszowa"/
    // "obojetnie" instead.
    const knownBrands = producentByTier[budzet] ?? [];
    const producentOptions = knownBrands.map((brand) => ({ id: brand, label: brand }));
    // Skip asking if the catalog has fewer than 2 well-known brands in this
    // tier yet — a single-option "który producent" isn't a real question
    // (same "skip if <2 meaningful options" rule as human). Multi-select
    // (up to MAX_PRODUCENT) so the verdict isn't locked to whichever single
    // brand happens to be tapped first - lib/wizardMatch.js's narrowPool()
    // hard-filters to ANY of the chosen brands and lets the normal ranking
    // decide the winner among them.
    if (producentOptions.length > 1) {
      ...
      if (!producentAnswer) return finish();
  ...
  if (needsBatteryVsPerf) {
    const options = [
      { id: "bateria", label: "Znikanie baterii w oczach" },
      { id: "wydajnosc", label: "Czekanie, aż telefon zacznie reagować" },
    ];
    const bw = push(
      "bateria_wydajnosc",
      "Co bardziej Cię wkurza w codziennym użytkowaniu?",
      options
    );
    if (!bw) return finish();
  ...
  // Asked regardless of whether ekran was picked as a priority above - that
  // only established how much it matters, not the direction - UNLESS this
  // tier's own recent catalog can't actually back up a compact-vs-large
  // choice (screenRangeByTier[budzet] === false, see
  // lib/wizardScreenRange.js): every answer would then land on the same
  // result, so asking would be a dishonest question. Any other value
  // (true, or missing because the caller didn't fetch it) asks as before.
  let rozmiar = null;
  if (screenRangeByTier[budzet] !== false) {
    rozmiar = push("rozmiar_ekranu", "Jakie gabaryty smartfona są dla Ciebie najwygodniejsze?", ROZMIAR_OPTIONS);
    if (!rozmiar) return finish();
