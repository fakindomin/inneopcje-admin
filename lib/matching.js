...
//     so "Lepiej" additionally requires the candidate to be in the same or
//     higher price_tier than base. Without that gate, a budget phone with a
//     great value score could get labeled "better" than a flagship it isn't
//     actually better than, which is exactly the kind of false claim this
//     engine exists to avoid. "Inaczej" similarly requires the SAME tier,
//     since its reason text explicitly claims "porównywalny poziom
//     jakości" (comparable quality) — that's only true within one segment.
...
function isSameOrHigherTier(base, candidate) {
...
// Picks the closest higher-scoring candidate that's also at least the same
// price tier — score alone isn't enough, since it's a value-for-money
// rating and a cheaper tier can out-score a pricier one on value without
// being "better."
function pickHigherScore(base, pool) {
  const baseScore = Number(base.score);
  const matches = pool.filter((c) => isSameOrHigherTier(base, c) && Number(c.score) > baseScore);
  if (matches.length === 0) return null;
