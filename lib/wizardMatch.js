...
const TAG_CEILING = { aparat: 200, bateria: 7400, wydajnosc: 5 };
...
export function scoreCandidate(candidate, profile) {
  ...
  // admin product form's raw specs JSON — no dedicated column yet). Soft
  // score, not a hard filter: this only ever gets asked in premium, but
  // there's no guarantee every premium refresh has a tagged foldable in
  // stock, and a hard filter would risk zero results the moment it doesn't.
  const isFoldable = candidate.specs?.foldable === true;
  if (profile.foldable === "skladany") {
    total += isFoldable ? 4 : -1;
  } else if (profile.foldable === "klasyczny" && isFoldable) {
    total -= 2;
  }
