...
// (1-5, the same scale chipset_tier is already hand-curated on) relative
// to the tier's own candidates, and comparing grades - "one of the best
// fifth in this tier" decides ties, not "the single absolute record
// holder", leaving a real shortlist for screen fit / brand / foldable /
// the catalog's own score to keep differentiating.
...
function buildGrader(candidates, tag) {
  const values = candidates.map((c) => tagValue(tag, c.specs)).sort((a, b) => a - b);
  const n = values.length;
  return (candidate) => {
    if (n === 0) return 0;
    const v = tagValue(tag, candidate.specs);
...
// preference is scored against. This only ever breaks a tie among
// candidates that already matched on every priority grade above it, so a
// single most-extreme screen size dominating isn't the same failure mode
// as it would be as a primary criterion.
...
export function pickBest(candidates, profile) {
  ...
  const graders = profile.tags.map((tag) => buildGrader(candidates, tag));
