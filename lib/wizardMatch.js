...
// app (lib/angles.js, components/AlternativeCard.js). At most ONE pick per
// angle (tansza/wyzsza_jakosc) - this isn't meant to ADD cards to the
// product page's existing 3-card Taniej/Lepiej/Inaczej grid, only to
// PREFER a selected-but-not-recommended brand over the generic cached pick
// for that same slot when one genuinely qualifies (see
// otherBrandPicksForProduct below). A brand with nothing cheaper or
// clearly better is simply left out - never forced in just to fill a slot.
// "Inaczej" is deliberately not replicated here; it's about niche-brand
// discovery, not a same-tier brand comparison.
function findOtherBrandPicks(recentPool, profile, winner) {
  ...
  for (const brand of otherBrands) {
    if (tanszaPick && lepiejPick) break;
    ...
    if (!tanszaPick && winnerPrice != null && altPrice != null && altPrice < winnerPrice) {
      tanszaPick = {
  ...
  return [tanszaPick, lepiejPick].filter(Boolean);
}
...
// Called from the product page (app/produkt/[slug]/page.js), not the
// wizard itself - the wizard passes its profile along in the "Zobacz pełne
// zestawienie" link (see components/PhoneWizard.js) instead of rendering
// these picks on its own verdict card, so they show up exactly once, in
// the same Taniej/Lepiej/Inaczej grid the product page already has, one
// click away like everything else there. `product` is that page's base
// product (i.e. the wizard's own winner) - price_tier drives which pool to
// search, same as matchTelefon().
export async function otherBrandPicksForProduct(product, profile) {
  if (!Array.isArray(profile?.producent) || profile.producent.length < 2) return [];

  const pool = getPool();
  const result = await pool.query(
    `SELECT p.id, p.name, p.slug, p.brand, p.brand_recognition, p.score, p.specs, p.price_tier, p.release_year
     FROM products p
     JOIN categories c ON c.id = p.category_id
     WHERE c.slug = 'telefony' AND p.status = 'published' AND p.price_tier = $1`,
    [product.price_tier]
  ...
  return findOtherBrandPicks(applyRecencyFilter(result.rows), profile, product);
}
