...
// jakosc_ekranu is a placeholder proxy, not real per-phone research: only
// specs.screen_panel_type ("AMOLED"/"LED", backfilled for a minority of
// the catalog so far) exists today. AMOLED outranks LED outranks unknown,
// which is directionally right but coarse - swap this for a real 1-5
// screen-quality score per phone as soon as that data lands (a parallel
// effort had it hand-researched for ~200 phones; not yet imported here).
function tagValue(tag, specs) {
  ...
  if (tag === "jakosc_ekranu") return specs?.screen_panel_type === "AMOLED" ? 2 : specs?.screen_panel_type === "LED" ? 1 : 0;
  return 0;
}
