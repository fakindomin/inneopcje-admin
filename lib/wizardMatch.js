...
// Keeps only candidates grading >=4/5 on `tag` (the top ~40% of the pool on
// that dimension) - but only if that still leaves at least MIN_POOL_SIZE
// candidates; otherwise the pool is left untouched for this tag. This is
// what makes a stated priority actually ELIMINATE phones from the pool
// (not just re-rank them, as tags alone did before), while still applying
// the same relax-rather-than-omit safety net already used for producent
// and release_year below - a thin tier/brand slice just won't narrow
// further once it's already close to the floor.
function narrowByTag(pool, tag, profile) {
  const grade = buildGrader(pool, tag, profile);
  const filtered = pool.filter((c) => grade(c) >= 4);
...
// (progressively narrower, each guarded by MIN_POOL_SIZE). Exported so the
// test counter can show the exact same pool pickBest() is about to rank
// from, not a separate approximation of it.
export function narrowPool(candidates, profile) {
  ...
  let pool = candidates;
  if (profile.marka === "tak" && profile.producent) {
    const brandMatches = candidates.filter((c) => c.brand === profile.producent);
    if (brandMatches.length > 0) pool = brandMatches;
  }

  // Recency is a hard filter too, but not a guaranteed-safe one like
  // producent above - a tier+brand slice can legitimately have nothing
  // released in the last two years, so this only narrows the pool when
  // doing so leaves at least one candidate; otherwise it falls back to
  // whatever producent/tier filtering already produced rather than ever
  // returning null.
  const minYear = minVerdictReleaseYear();
  const recentMatches = pool.filter((c) => Number(c.release_year) >= minYear);
  if (recentMatches.length > 0) pool = recentMatches;

  for (const tag of profile.tags ?? []) {
    pool = narrowByTag(pool, tag, profile);
  }
