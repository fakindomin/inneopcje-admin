...
// `model` override is for the same route's ?model= - every key returning
// an identical 429 points at the MODEL having no free quota right now
// (common for a freshly-released preview tier), not at the keys/accounts.
export async function generatePriceText(name, apiKey, model = MODEL) {
