...
const CATEGORY_SPEC_HINTS = {
  telefony:
    'Dla telefonu pole "specs" MUSI zawierać: screen_size_inches (number), chipset (string, ' +
    "pełna nazwa układu np. \"Snapdragon 8 Elite Gen 5\"), ram_gb (number), camera_main_mp (number), " +
    "battery_mah (number), front_camera_mp (number, aparat przedni/selfie), " +
    "charging_w (number, maksymalna moc ładowania przewodowego w watach), " +
    'screen_panel_type ("LED" albo "AMOLED" - dla OLED/Super AMOLED/Dynamic AMOLED/P-OLED wpisz ' +
    '"AMOLED", dla IPS LCD/TFT/zwykłego LCD wpisz "LED"), weight_g (number, waga w gramach), ' +
    'foldable (boolean, true tylko dla telefonów składanych). Opcjonalnie: ip_rating (string, np. ' +
    '"IP68" - pomiń to pole całkowicie, jeśli telefon nie ma oficjalnego certyfikatu odporności). ' +
    "Jeśli nie możesz zweryfikować którejś z tych wartości, oszacuj ją na podstawie segmentu i roku " +
    "premiery zamiast pomijać pole - z wyjątkiem ip_rating, które ma zostać pominięte, gdy naprawdę go nie ma.",
  telewizory:
