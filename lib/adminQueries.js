...
export async function listFailedQueue() {
  ...
  try {
    const { rows } = await pool.query(
      `SELECT id, category, product_name, created_at
    ...
    // has run at least once against this database.
...
export async function listRecentRuns(limit = 15) {
  ...
  try {
    const { rows } = await pool.query(
      `SELECT id, category, status, published_count, draft_count, failed_count, started_at, finished_at
