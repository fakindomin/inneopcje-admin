...
// Used when a draft gets published from the main admin panel — bot- and
// manual-import-inserted drafts alike only get alternatives computed at
// publish time (never at insert time as a draft), so this needs to be
// called wherever a product's status flips to 'published'.
export async function linkAlternativesForProduct(productId) {
  const pool = getPool();
  const { rows } = await pool.query(
    `SELECT id, category_id, slug, name, brand, brand_recognition, score, price_tier, specs
     FROM products WHERE id = $1`,
    [productId]
  );
  const product = rows[0];
  if (!product) return 0;
  return linkManualAlternatives(product.category_id, product);
}
