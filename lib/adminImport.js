...
// Slug is intentionally left untouched on edit — it's the public URL, and
// changing it on every name tweak would break existing links/SEO. Only
// normalized_name (used for search + the import dedup check) is refreshed.
export async function updateProduct(id, categoryId, data) {
  const pool = getPool();
  await pool.query(
    `UPDATE products SET
       category_id = $1, name = $2, normalized_name = $3, brand = $4, brand_recognition = $5,
       verdict = $6, score = $7, summary = $8, pros = $9::jsonb, cons = $10::jsonb, specs = $11::jsonb,
       price_tier = $12, release_year = $13, updated_at = now()
     WHERE id = $14`,
    [
      categoryId,
      data.name,
      normalizeQuery(data.name),
      data.brand,
